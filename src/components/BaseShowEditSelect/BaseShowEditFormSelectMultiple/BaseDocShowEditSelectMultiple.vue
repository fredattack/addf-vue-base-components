<template>
  ...
  <div v-else class='mt-3 flex flex-col '>
    <BaseShowLabel :label="label ? label : $t('attributes.available_docs')" :required="required"/>
    <div v-if='metadata && metadata.doc_types && modelValue.length >= 1'>
      <span v-for="(item, index) in modelValue" class="inline-block mr-3 capitalize-first" :key="index">
